...
void kmp(char T[], char P[], int tamT, int tamP)
{
	...
	int p = 0; // var aux das ocorrencias
	int k = 1; // var aux das ocorrencias
	while(i < tamT)
	{
		if(T[i] == P[j]) // match
		{
			...
			{
				if(k == 1 && p == 0 ) // condicao para mostrar apenas a primeira ocorrencia
				{
					cout << i - tamP << endl;
					j = lps[j - 1];
					p ++;
		...
		else // mismatch
		{
			...
			{
				i++;
				if (i == tamT && p == 0 )  //caso o padrao todo seja percorrido e nenhuma ocorrencia foi detectada
				{
					cout << "Email incorreto" << endl; // Saida de dados
				}
			}
		}
