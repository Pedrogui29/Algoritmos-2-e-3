...
int main()
{
	...
	{
		aux++;
		// cout << fecho_convexo.back().x << " " << fecho_convexo.back().y << endl;
		fecho_convexo.pop_back();
		
		
		if(p == 0)
		{
			a = fabs(fabs(fecho_convexo.back().x) + fabs(fecho_convexo.back().y)) + a;
			p++;
		}
		if (p == 1)
		{
			b = fabs(fabs(fecho_convexo.back().x) + fabs(fecho_convexo.back().y)) + b;
			p++;
		}
	...
	c = sqrt((a*a) + (b*b)); // calculo de c pela hipotenusa 
	


   perimetro = a+b+c; // calculo perimetro
   
   // saida de dados 
   cout << fixed << setprecision(2) << perimetro <<" m" <<endl; 
